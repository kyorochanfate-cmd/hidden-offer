...
import { el, clear, loop, clamp } from "../dom.js?v=1.0.9";
import { Store } from "../state.js?v=1.0.9";
import { Router } from "../app.js?v=1.0.9";
import { finishGame } from "./result.js?v=1.0.9";
...
const PROBLEMS = [
  ...
  { text: "来週月曜日は直行します。午前中はチャットのみ対応可能です。", answer: "like", sender: "同僚 田中" },
  ...
  { text: "新しいスローガンは『アジャイル熱盛』！ …ちょっとスベったかね？", answer: "funny", sender: "佐藤部長" },
...
export function startChatRally(mount, gameId) {
  ...
  function addBossMessage(sender, text) {
    ...
    const avatarContent = isBoss 
      ? el("img", { src: "assets/img/boss.png", alt: "B" })
